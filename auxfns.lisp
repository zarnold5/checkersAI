...
(defmacro defresource (name &key constructor (initial-copies 0)
  ...
  (let ((resource (symbol2 '* (symbol2 name '-resource*)))
        (deallocate (symbol2 'deallocate- name))
        (allocate (symbol2 'allocate- name)))
    `(progn
...
(defmacro with-resource ((var resource &optional protect) &rest body)
  ...
  (let ((allocate (symbol2 'allocate- resource))
        (deallocate (symbol2 'deallocate- resource)))
    (if protect
        `(let ((,var nil))
...
(defmacro define-enumerated-type (type &rest elements)
  ...
  `(progn
     ...
     (defun ,(symbol2 type '->symbol) (,type)
       (elt ',elements ,type))
     (defun ,(symbol2 'symbol-> type) (symbol)
